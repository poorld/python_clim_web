    ...
    def execute(self, keywords):
        ...
        try:
            ...
            for round_num in range(1, max_rounds + 1):
                ...
                logger.info(f"📊 第 {round_num} 轮检测完成：{success_count}/{len(keywords)} 个关键词有货并下单")
            ...
            logger.info("🎉 单独建单多轮检测完成")

        except Exception as e:
            logger.error(f"❌ 单独建单多轮检测出错: {e}", exc_info=True)
